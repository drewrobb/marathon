...
class MarathonSchedulerActor(
  ...
  def awaitCancellation(plan: DeploymentPlan, origSender: ActorRef): Receive =
    sharedHandlers.andThen(_ => tryDeploy(plan, origSender)) orElse {
      case _ => stash()
    }
  ...
  /**
    ...
    */
  def tryDeploy(plan: DeploymentPlan, origSender: ActorRef): Unit = {
  ...
  def deploy(origSender: ActorRef, cmd: Deploy): Unit = {
    ...
    res match {
      ...
      case Failure(e: LockingFailedException) if cmd.force =>
        ...
        context.become(awaitCancellation(plan, origSender))
